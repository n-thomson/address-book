...
describe("ContactController", () => {
  ...
  describe('#addContact()', () => {
    it('should add a single contact into the book', (done) =>{
      this.book.addContact('Alice', '001-101-1010', 'alice@testmail.com')
      .then((contact) => {
        expect(contact.name).toBe('Alice');
        expect(contact.phone).toBe('001-101-1010');
        expect(contact.email).toBe('alice@testmail.com');
        done();
      })
      .catch((err) => {
        done();
  ...
  describe("search methods", () =>{
    ...
    describe('#iterativeSearch()', () => {
      it('should return null when the contact is not found', (done) =>{
    ...
    it('should remove the contact that matches the id passed', (done) => {
      ...
      .then(() => {
        ...
        .then((contacts) => {
          expect(contacts[0].name).toBe("Rick Deckard");
          expect(contacts.length).toBe(1);
          this.book.delete(contacts[0].id)
          .then(() => {
            this.book.getContacts()
            .then((contacts) => {
              expect(contacts.length).toBe(0);
